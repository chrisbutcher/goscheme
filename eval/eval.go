...
func Eval(expression types.Expression, en *types.Environment) (types.Expression, error) {
  ...
  case []types.Expression:
    ...
    case "set!":
      car := exp[1].(types.Symbol)
      if evaluated, err := Eval(exp[2], en); err == nil {
        if found_env := env.Get(car, en); found_env != nil {
          found_env.EnvVars[car] = evaluated
          return car, nil
        } else {
          return nil, errors.New(fmt.Sprint("Symbol not found in environment: ", car))
        }
      } else {
        return nil, nil
      }
